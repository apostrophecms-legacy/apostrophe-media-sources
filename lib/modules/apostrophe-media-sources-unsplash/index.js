...
module.exports = {
  ...
  construct (self, options) {
    ...
    self.getAndWriteFile = async (file) => {
    ...
    self.unlinkImages = async (tempPaths) => {
    ...
    self.download = async (req, files) => {
      ...
      for (const file of files) {
        const fileInfos = await self.getAndWriteFile(file);
        const image = await self.insertImage(req, fileInfos);

        tempPaths.push(fileInfos.tempPath);
        images.push(image);
      }
